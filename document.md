...
Similarly, the join query can also expressed as:

For instance, the above join queries could be expressed by a `UNNEST` clause:
...
Note that all the three appearance of the same join query yeld equivalently efficeint query execution plans generated by AsterixDB query optimizer. In fact, all join queries could be expressed by `UNNEST` clauses and all left outer join queries could be expressed by `LEFT OUTER UNNEST`s.
