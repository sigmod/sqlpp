...
###  <a id="SELECT_Statement">SELECT Statement
	...
	SelectBlock	::=	( SelectClause ( FromClause ( WithClause )? )? (WhereClause )? ( GroupbyClause ( WithClause )? ( HavingClause )? )? | FromClause ( WithClause )? ( WhereClause )? ( GroupbyClause ( WithClause )? ( HavingClause )? )? SelectClause )
