...
SQL++ allows correlations among different from terms, i.e., a right side `From` binding expression can refer to variables defined on its left side. A equivalen query to the above unnesting query could be rewritten as:
...
Replacing `UNNEST` with `,` the same join intent of the `UNNEST`-based join queries could be expressed by:
...
The first query is one of the SQL-92 ways to express the join intent.
...
Note that all the five appearances of the same join intent so far yeld equivalently efficeint query execution plans generated by AsterixDB query optimizer. In fact, all join queries could be expressed by `UNNEST` clauses and all left outer join queries could be expressed by `LEFT OUTER UNNESTs`.
